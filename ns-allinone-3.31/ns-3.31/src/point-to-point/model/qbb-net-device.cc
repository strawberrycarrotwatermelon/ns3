...
QbbNetDevice::DequeueAndTransmit (void)
{
	...
	{	
		...
		if (m_node->GetNodeType()==0 && m_qcnEnabled) //nothing to send possible due to qcn flow control, if so reschedule sending
		{
			...
			if (m_node->GetNodeType() == 0) //NIC
			{
				if (ipv4h.GetProtocol() == 17)	//look at udp only
				{
					...
					if (x == 2) //generate NACK
					{
						...
						head.SetIdentification(UniformVariable().GetValue(0, 65536));
						newp->AddHeader(head);
...
QbbNetDevice::CheckQueueFull(uint32_t inDev, uint32_t qIndex)
{
	...
	for (uint32_t j=0;j<qCnt;j++)
	{
		if (pClasses[j])			// Create the PAUSE packet
		{
			...
			ipv4h.SetTtl(1); 
			ipv4h.SetIdentification(UniformVariable().GetValue(0, 65536));
			p->AddHeader(ipv4h);
...
void
QbbNetDevice::SetQueue (Ptr<BEgressQueue<Packet>> q)
{
	NS_LOG_FUNCTION (this << q);
	m_queue = q;
}

Ptr<BEgressQueue<Packet>>
...
QbbNetDevice::rpr_active_byte(uint32_t fIndex, uint32_t hop)
{
	...
	if (m_rpByteStage[fIndex][hop] < m_rpgThreshold)
		//m_txBytes[fIndex][hop] = m_bc*UniformVariable(0.85,1.15).GetValue();
		m_txBytes[fIndex][hop] = m_bc;
	else
		//m_txBytes[fIndex][hop] = m_bc*UniformVariable(0.85,1.15).GetValue()/2;
		m_txBytes[fIndex][hop] = m_bc;
	//for fluid model
...
QbbNetDevice::rpr_active_time(uint32_t fIndex, uint32_t hop)
{
	...
	if (m_rpTimeStage[fIndex][hop]<m_rpgThreshold)
		//m_rpWhile[fIndex][hop] = m_rpgTimeReset*UniformVariable(0.85,1.15).GetValue();
		m_rpWhile[fIndex][hop] = m_rpgTimeReset;
	else
		//m_rpWhile[fIndex][hop] = m_rpgTimeReset*UniformVariable(0.85,1.15).GetValue()/2;
		m_rpWhile[fIndex][hop] = m_rpgTimeReset;
